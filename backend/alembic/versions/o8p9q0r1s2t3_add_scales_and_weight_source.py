...
# postgresql.ENUM(create_type=False) — jadval yaratishda qayta CREATE qilmaydi
scale_type_enum    = postgresql.ENUM('manual', 'serial', 'api',                         name='scale_type',   create_type=False)
scale_status_enum  = postgresql.ENUM('active', 'inactive', 'error',                     name='scale_status', create_type=False)
weight_source_enum = postgresql.ENUM('camera_ai', 'manual', 'scale_serial', 'scale_api', name='weight_source', create_type=False)


def upgrade() -> None:
    # ─── 1. Enum turlarini DO bloki bilan xavfsiz yaratish ───────────────────
    op.execute("""
        DO $$ BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'scale_type') THEN
                CREATE TYPE scale_type AS ENUM ('manual', 'serial', 'api');
            END IF;
        END $$;
    """)
    op.execute("""
        DO $$ BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'scale_status') THEN
                CREATE TYPE scale_status AS ENUM ('active', 'inactive', 'error');
            END IF;
        END $$;
    """)
    op.execute("""
        DO $$ BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'weight_source') THEN
                CREATE TYPE weight_source AS ENUM ('camera_ai', 'manual', 'scale_serial', 'scale_api');
            END IF;
        END $$;
    """)
    ...
    op.create_table(
        ...
        sa.Column('scale_type',               scale_type_enum,   nullable=False, server_default='manual'),
        sa.Column('location',                 sa.String(200),    nullable=True),
        sa.Column('status',                   scale_status_enum, nullable=False, server_default='active'),
    ...
    op.add_column('weight_measurements', sa.Column(
        'source', weight_source_enum, nullable=False, server_default='camera_ai',
    ))
    op.add_column('weight_measurements', sa.Column('actual_weight_kg', sa.Float(), nullable=True))
    op.add_column('weight_measurements', sa.Column(
        'scale_id', sa.Integer(), sa.ForeignKey('scales.id', ondelete='SET NULL'), nullable=True,
    ))
    op.add_column('weight_measurements', sa.Column('notes', sa.Text(), nullable=True))
