/**
 * AnimalsPage — Sprint 7-8
 *
 * /api/v1/animals/search endpointidan foydalanadi:
 *   - Species, gender, status filterlari
 *   - Sort (tag_id, species, status, total_detections, last_detected_at)
...
export default function AnimalsPage() {
  ...
  const loadAnimals = useCallback(async () => {
    ...
    try {
      ...
      const data = await apiFetch<any>(`/api/v1/animals/search?${params}`);

      // Backend array yoki { items, total } qaytarishi mumkin
  ...
  }, [page, sortBy, sortOrder, debouncedSearch, species, gender, statusFilter]);
  ...
  async function handleDelete(animal: Animal) {
    ...
    try {
      ...
      loadAnimals();
    } catch (e) { alert("O'chirish xatolik: " + (e instanceof Error ? e.message : '')); }
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {loading && animals.length === 0 ? 'Yuklanmoqda...' : total > 0 ? `Jami ${total} ta jonivor` : "Ro'yxat bo'sh"}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={loadAnimals} disabled={loading}
      ...
      {showAdd && <AnimalModal onClose={() => setShowAdd(false)} onSaved={() => { setShowAdd(false); loadAnimals(); }} />}
      {editAnimal && <AnimalModal initial={editAnimal} onClose={() => setEditAnimal(null)} onSaved={() => { setEditAnimal(null); loadAnimals(); }} />}
    </div>
  );
}